...
export default function ApiKeysClient({ baseUrl, spec }: { baseUrl: string; spec: any | null }) {
  ...
  async function act(key: KeyRow, action: 'revoke' | 'rotate') {
    ...
    try {
      ...
      await refresh();
    } catch (error) { setMessage(error instanceof Error ? error.message : `Could not ${action} key.`); }
    finally { setBusy(false); }
  }
  async function copy() {
  ...
  return <div className="space-y-7">
    <section className="rounded-2xl border bg-card p-5 sm:p-7"><div className="flex items-start gap-3"><ShieldAlert className="mt-1 shrink-0 text-primary" size={20}/><div><h2 className="text-xl font-semibold">Create a scoped key</h2><p className="mt-1 text-sm text-muted-foreground">Use the minimum permissions your integration needs. Keep the token in a secrets manager or platform credential store.</p></div></div>
      ...
      <button disabled={busy || !name.trim() || !scopes.length || Boolean(info?.is_admin && !operatorId)} onClick={create} className="mt-5 rounded-xl bg-primary px-5 py-3 font-semibold text-primary-foreground disabled:opacity-50">Create key</button>
    ...
    <section className="rounded-2xl border bg-card p-5 sm:p-7"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Issued keys</h2><button aria-label="Refresh keys" onClick={() => void refresh()} className="text-primary"><RefreshCw size={18}/></button></div><div className="mt-4 space-y-3">{info?.keys.map((key) => <div key={key.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">{key.name} <span className="ml-2 rounded-full bg-muted px-2 py-1 text-xs font-normal">{key.status}</span></h3><p className="mt-1 font-mono text-xs text-muted-foreground">{key.key_prefix}…</p></div>{key.status === 'active' && <div className="flex gap-2"><button disabled={busy} onClick={() => void act(key, 'rotate')} className="rounded-lg border px-3 py-2 text-xs hover:bg-muted"><RefreshCw size={13} className="mr-1 inline"/>Rotate</button><button disabled={busy} onClick={() => void act(key, 'revoke')} className="rounded-lg border px-3 py-2 text-xs text-destructive hover:bg-muted"><Trash2 size={13} className="mr-1 inline"/>Revoke</button></div>}</div><p className="mt-3 text-xs text-muted-foreground">Owner: {key.owner_email ?? key.user_id} · Operator: {key.operator_id}</p><p className="mt-1 text-xs text-muted-foreground">Created: {dt(key.created_at)} · Expires: {dt(key.expires_at)} · Last used: {dt(key.last_used_at)}</p><div className="mt-2 flex flex-wrap gap-1">{key.scopes.map((scope) => <span key={scope} className="rounded bg-primary/10 px-2 py-1 text-xs text-primary">{scope}</span>)}</div></div>)}{info && !info.keys.length && <p className="text-sm text-muted-foreground">No scoped API keys yet.</p>}</div></section>
